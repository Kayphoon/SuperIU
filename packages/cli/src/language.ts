...
const DICTS: Record<UiLanguage, Record<string, string>> = {
  zh: {
    ...
    'cli.new.ok': '✔ 新会话 {id}',
    'cli.clear.ok': '✔ 已追加 reset_boundary；当前上下文已清空。',
    'cli.memory.analyzing': '正在分析当前对话以提取持久事实…',
    'cli.memory.none': '没有新的内容需要记住。',
    'cli.memory.failed': '✖ 记忆提取失败：{message}',

    'cli.help.header': '可用斜杠命令：',
    'cli.help.status': '显示状态、会话 ID、JSONL 路径、叶节点 ID、分支长度',
    'cli.help.clear': '追加 reset_boundary 并清空当前上下文',
    'cli.help.history': '显示最近的提示词历史（可附加过滤词）',
    'cli.help.sessions': '列出当前工作区的 JSONL 会话',
    'cli.help.load': '按 ID、文件名或路径载入会话',
    'cli.help.new': '开启一个新的 JSONL 会话',
    'cli.help.memory': '从本次对话中提取持久事实写入 MEMORY.md / USER.md',
  ...
  en: {
    ...
    'cli.memoryDir': 'Memory directory: {path}',
    ...
    'cli.clear.ok': '✔ reset_boundary appended; context truncated to empty.',
    ...
    'cli.help.status': 'Show state, session id, JSONL path, leaf id, branch length',
    'cli.help.clear': 'Append reset_boundary and truncate the active context',
    'cli.help.history': 'Show recent prompt history (optionally filtered)',
    'cli.help.sessions': 'List JSONL sessions for this workspace',
    'cli.help.load': 'Load a session by id, file name, or path',
    'cli.help.new': 'Start a fresh JSONL session',
    'cli.help.memory': 'Extract durable facts from this conversation into MEMORY.md / USER.md',

...
- **本地化守卫 (`scripts/check-ui-i18n.mjs`，已接入 `pnpm test`)**: 内联 SPA 的模块体 `tsc` 不解析，故硬编码文案只能靠脚本兜住。已知两类缺陷——(1) 硬编码中文；(2) **硬编码英文**拼接进 DOM（`apiKeyMasked + ' (unchanged)'`，纯 ASCII，只查 CJK 的扫描看不见，中文界面里直接漏出英文）。守卫按**函数名大括号匹配**只截取设置面板与服务商主从区域的 12 个函数（约 206 行），不做全文件散文启发式——全量扫描会把「关于」页既有的环境变量快照（`OPENAI_BASE_URL: `、`(unset)`）与诊断串一并报成噪音，逼人加白名单或越界 i18n 化。判定要点：`\s` 必须测**原始值**而非 `trim()` 后（`' (unchanged)'` 的空格只在边缘，trim 后成单词会被放过——这正是该守卫要抓的那一个 bug）；`notify.*` 四个标题**故意双语**（macOS 通知横幅在页面语言上下文之外渲染），已在 en 表 CJK 检查中显式豁免。脚本自带 `self-test` 断言该回归可被检出，避免提取器落空导致空转全绿。
